...
version_choose () {
    ...
    case $version_num in
        0)
            ...
            ;;
        1)
            echo -e '\e[92m已选择Docker_容器版\e[0m' && version_num=4
            ;;
        2)
            echo -e '\e[92m已选择Stable_稳定版\e[0m' && version_num=3
            ;;
        3)
            echo -e '\e[92m已选择Formal_正式版\e[0m' && version_num=2
            ;;
        *)
...
repo_set () {
    ...
    repo_url=https://github.com/DHDAXCW/DoorNet2/releases
    firmware_id=openwrt-rockchip-armv8-embedfire_doornet2-${format}-sysupgrade.img
}
#寻找固件
search_file () {
